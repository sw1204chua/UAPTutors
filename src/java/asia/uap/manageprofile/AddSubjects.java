 ...
 * @author Hawy
 */
@WebServlet(name = "AddSubjects", urlPatterns = {"/profile/do.addsubject"})
public class AddSubjects extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Boolean error = false;
        if(subjectname == null || subjectname.isEmpty() || category == null || category.isEmpty() || user == null){
            error = true;
        }
        if (!error){
            try{               
                dbOp.addUserSubjects(subjectname, category, user.getId());
                User updatedUser = dbOp.getUser(user.getId());
                session.setAttribute("current", updatedUser);
            }catch(SQLException ex){
                ex.printStackTrace();
            }
        }
        String uri = "/WEB-INF/result.jsp";
        String successmsg = "Subject has been added!";
